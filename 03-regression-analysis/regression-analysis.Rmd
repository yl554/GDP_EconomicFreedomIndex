...
Model selecting using AIC: logGDP ~ TariffRate + logpop + GDPGrowth + TaxBurdenCent + GovSpendingCent + cat_inflationDangerouslyHigh + cat_inflationHigh + cat_inflationLow + RegionAsia-Pacific + RegionEurope + RegionMiddleEastandNorthAfrica + RegionSub-SaharanAfrica + GovInterferenceLimited + GovInterferenceExtensive + GovInterferenceRepressive
    
Model selecting using BIC: logGDP ~  TariffRate + logpop + GDPGrowth + cat_inflationDangerouslyHigh + cat_inflationHigh + GovInterferenceExtensive + RegionSub-Saharan Africa             

Model selecting using adj R^2: logGDP ~ TariffRate + logpop + GDPGrowth + TaxBurdenCent +  cat_inflationDangerously High + cat_inflationHigh + GovInterferenceLimited + GovInterferenceModerate + RegionSub-Saharan Africa 
...
```{r Nested F for interaction with cat_inflation, echo=TRUE}
reduced <- lm(logGDP ~ TariffRate + logpop + GDPGrowth + cat_inflation + GovInterference + Region, data = economic_data)
...
```{r Nested F for interaction with GovInterference, echo=TRUE}
reduced <- lm(logGDP ~ TariffRate + logpop + GDPGrowth + cat_inflation + GovInterference + Region, data = economic_data)
...
kable(anova(reduced, full), format="markdown", digits = 3)
```
```{r Nested F for interaction with region , echo=TRUE}
reduced <- lm(logGDP ~ TariffRate + logpop + GDPGrowth + cat_inflation + GovInterference + Region, data = economic_data)
