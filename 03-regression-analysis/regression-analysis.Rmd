...
As the Tariff Rate increases by one percentage point, the expected GDP of the country is multiplied by a factor of `r exp(-.096)`, holding all other variables constant. As the tariff rate increases, we can expect the GDP of the country to be lower.

As the Corporate Tax Rate increases by one percentage point, the expected GDP of the country is multiplied by a factor of `r exp(-.036)`, holding all other variables constant. As the corporate tax rate increases, we can expect the GDP of the country to be lower.

As the Tax Burden increases by one percentage point, the expected GDP of the country is multiplied by a factor of `r exp(.024)`, holding all other variables constant. As the tax burden increases, we can expect the GDP of the country to be higher.

As the rate of GDP growth increases by one percentage point, the expected GDP of the country is multiplied by a factor of `r exp(-.098)`, holding all other variables constant. As the rate of GDP growth increases, we can expect the GDP of the country to be lower.

If the country's inflation rate is listed as "Healthy", this provides no further input to the model as "Healthy" is the baseline for our model. However, if the country's inflation is listed as low, the expected GDP of the country is multiplied by a factor of `r exp(-.004)`, holding all other variables constant. If the country's inflation is listed as high, the expected GDP of the country is multiplied by a factor of `r exp(-.488)`, holding all other variables constant. If the country's inflation is listed as dangerously high, the expected GDP of the country is multiplied by a factor of `r exp(-.723)`, holding all other variables constant. 
